...
def log_result(name, passed, query_ms, total_ms, error=None):
    test_results.append({
        "Test": name,
...
# --- Individual Tests ---
def timed_test(func):
    """Decorator to measure both query and total time per test."""
    def wrapper(test_name):
        ...
        try:
            # start inner timing before report function call
            query_start = time.time()
            func()  # executes the test and report query
            query_end = time.time()
            ...
            print(f"   ↳ Query executed in {query_ms:.2f} ms")
            print(f"✓ {test_name} passed in {total_ms:.2f} ms\n")
            log_result(test_name, True, query_ms, total_ms)
        except AssertionError as e:
            total_end = time.time()
            query_end = query_end or time.time()
            query_ms = (query_end - query_start) * 1000
            total_ms = (total_end - total_start) * 1000
            print(f"   ↳ Query executed in {query_ms:.2f} ms")
            print(f"  {test_name} failed ({total_ms:.2f} ms): {e}\n")
            log_result(test_name, False, query_ms, total_ms, str(e))
        except Exception as e:
            total_end = time.time()
            query_end = query_end or time.time()
            query_ms = (query_end - query_start) * 1000
            total_ms = (total_end - total_start) * 1000
            print(f"   ↳ Query executed in {query_ms:.2f} ms")
            print(f"  {test_name} error ({total_ms:.2f} ms): {e}\n")
            log_result(test_name, False, query_ms, total_ms, str(e))
    return wrapper
...
def test_report_functions_availability():
    ...
    for rep_name, func in reports.items():
        ...
        print(f"  ✓ {rep_name}: {len(df)} records ({query_ms:.2f} ms)")
...
def run_all_tests():
    ...
    # --- Summary Table ---
    print("=" * 60)
    print(" TEST SUMMARY (Query vs Total Time in ms):")
    for res in test_results:
        color = GREEN if res["Result"] == "PASS" else RED
        print(f"{color}{res['Result']:<6}{RESET} {res['Test']:<40} "
              f"Query: {res['Query (ms)']:.2f} ms | Total: {res['Total (ms)']:.2f} ms")
        if res["Error"]:
    ...
    print(f"  Total suite time: {total_time:.2f} ms")
